...
async function checkNuevoEstudiante2() {
    ...
    try {
        ...
        const [projectsAsMain] = await connection.execute(`
            SELECT p.id, p.nombre, p.descripcion, p.estudiante_id
            FROM proyectos p
        ...
        const [projectsAsUser] = await connection.execute(`
            SELECT p.id, p.nombre, p.descripcion, pu.rol
            FROM proyectos p
        ...
        if (projectsAsUser.length === 0) {
            ...
            projectsAsUser.forEach(project => {
                console.log(`   - ID: ${project.id}, Nombre: ${project.nombre}, Rol: ${project.rol}`);
            });
            console.log('');
        }
        ...
        if (deliverables.length === 0) {
            ...
            deliverables.forEach(deliverable => {
                ...
                console.log(`     Proyecto: ${deliverable.proyecto_nombre} (ID: ${deliverable.proyecto_id})`);
